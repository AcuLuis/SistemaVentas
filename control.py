...
def busca_cliente():
    ...
    consulta = f"""
    SELECT * FROM cliente
    WHERE cod_cliente LIKE '%{ui.txt_busca_bd_cliente.text()}%' OR nombre LIKE '%{ui.txt_busca_bd_cliente.text()}%' OR dni = '{ui.txt_busca_bd_cliente.text()}';
    """
...
def busca_producto():
    ...
    WHERE cod_producto LIKE '%{ui.txt_busca_bd_producto.text()}%' OR tipo LIKE '%{ui.txt_busca_bd_producto.text()}%' OR marca = '{ui.txt_busca_bd_producto.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_cliente_actualizar():
    ...
    consulta = f"""
    SELECT * FROM cliente
    WHERE cod_cliente LIKE '%{ui.txt_busca_actualiza_cliente.text()}%' OR nombre LIKE '%{ui.txt_busca_actualiza_cliente.text()}%' OR dni = '{ui.txt_busca_actualiza_cliente.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_producto_actualizar():
    ...
    consulta = f"""
    SELECT * FROM producto
    WHERE cod_producto LIKE '%{ui.txt_busca_actualiza_producto.text()}%' OR tipo LIKE '%{ui.txt_busca_actualiza_producto.text()}%' OR marca = '{ui.txt_busca_actualiza_producto.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_cliente_eliminar():
    ...
    consulta = f"""
    SELECT * FROM cliente
    WHERE cod_cliente LIKE '%{ui.txt_busca_elimina_cliente.text()}%' OR nombre LIKE '%{ui.txt_busca_elimina_cliente.text()}%' OR dni = '{ui.txt_busca_elimina_cliente.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_producto_eliminar():
    ...
    consulta = f"""
    SELECT * FROM producto
    WHERE cod_producto LIKE '%{ui.txt_busca_elimina_producto.text()}%' OR tipo LIKE '%{ui.txt_busca_elimina_producto.text()}%' OR marca = '{ui.txt_busca_elimina_producto.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_cliente_venta():
    ...
    consulta = f"""
    SELECT * FROM cliente
    WHERE cod_cliente LIKE '%{ui.txt_busca_venta_cliente.text()}%' OR nombre LIKE '%{ui.txt_busca_venta_cliente.text()}%' OR dni = '{ui.txt_busca_venta_cliente.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def busca_producto_venta():
    ...
    consulta = f"""
    SELECT * FROM producto
    WHERE cod_producto LIKE '%{ui.txt_busca_venta_producto.text()}%' OR tipo LIKE '%{ui.txt_busca_venta_producto.text()}%' OR marca = '{ui.txt_busca_venta_producto.text()}';
    """
    cursor.execute(consulta)
    datos = cursor.fetchall()
    if datos:  # Verifica si hay datos antes de procesarlos
...
def refresca_data():
    try:
        ...
        query = """
            SELECT num_venta, fecha, tipo, cod_cliente, cod_producto, cantidad
            FROM det_venta
...
def verifica_fechas_semanal():
    if (
        ui.cmb_reporte_semanal_anio.currentIndex() == 0
...
def generar_reporte_semanal():
    if verifica_fechas_semanal()== True:
        ...
        try:
            ...
            query = f"""
                SELECT det_venta.num_venta, det_venta.fecha, producto.tipo, cliente.nombre
                FROM det_venta
